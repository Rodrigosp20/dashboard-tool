...
if 'df_todas_dimen' and 'df_dados_setor_todas' and 'df_comparacao' in st.session_state:
    ...
    # coluna do medidor e do gráfico de linhas.
    col1_mid, col2_mid = st.columns([1, 3])

    # coluna do gráfico de barras e do texto.
    col1_bot, col2_bot = st.columns([2, 1])

    # configuração para desabilitar o logo do plotly em cada gráfico
    ...
    titulo_principal = st.sidebar.text_input(
        "Titulo principal", "Rentabilidade do negócio")

    st.sidebar.markdown("***")  # Cria uma linha na sidebar

    # todos os anos presentes na dataframe Comparação
    years_graph_gauge = st.session_state.df_comparacao.columns[3:]
    ...
    titulo_gauge = st.sidebar.text_input(
        "Titulo medidor", "Desempenho agregado entre "+str(year_slider_gauge[0])+" e "+str(year_slider_gauge[1]))

    st.sidebar.markdown("***")

    # caixa de texto para mudar o título do gráfico de linhas
    titulo_linhas = st.sidebar.text_input(
        "Titulo gráfico linhas", "Diferença entre o desempenho da empresa e a média do setor")

    # color picker para escolher a cor do gráfico de linhas
    color_1 = st.sidebar.color_picker(
        'Cor do grafico de linhas', '#192646', key=1)

    # todos os anos presentes na dataframe Comparação
    years_graph_linhas = st.session_state.df_comparacao.columns[3:]
    ...
    titulo_barras = st.sidebar.text_input(
        "Título gráfico de barras", st.session_state.option)
    ...
    color_2 = st.sidebar.color_picker(
        'Cor do grafico de barras para a empresa', '#192646', key=2)
    color_3 = st.sidebar.color_picker(
        'Cor do grafico de barras para a média do setor', '#959fb8', key=3)

    # todos os anos presentes na dataframe Comparação
    years_graph_barras = st.session_state.df_comparacao.columns[3:]
    ...
    def get_(key, col):
        try:
            return st.session_state.df_todas_dimen.loc[key, col]
        except KeyError:
            return np.nan
    ...
    def medias_per_year(years):
        ...
        for year in years:
            ...
            if st.session_state.option == 'Liquidez imediata':
                quartil_1.append(get_('Caixa e depósitos bancários',
                                      q1) / get_('Passivo corrente', q1))
                mediana.append(get_('Caixa e depósitos bancários',
                                    med) / get_('Passivo corrente', med))
                quartil_3.append(get_('Caixa e depósitos bancários',
                                      q3) / get_('Passivo corrente', q3))

            elif st.session_state.option == 'Endividamento':
                quartil_1.append(get_('Passivo', q1) / get_('Ativo', q1))
                mediana.append(get_('Passivo', med) / get_('Ativo', med))
                quartil_3.append(get_('Passivo', q3) / get_('Ativo', q3))

            elif st.session_state.option == 'Solvabilidade':
                quartil_1.append(get_('Ativo', q1) / get_('Passivo', q1))
                mediana.append(get_('Ativo', med) / get_('Passivo', med))
                quartil_3.append(get_('Ativo', q3) / get_('Passivo', q3))

            elif st.session_state.option == 'Alavancagem Financeira':
                quartil_1.append(get_('Financiamentos obtidos', q1) /
                                 (get_('Financiamentos obtidos', q1) + get_('Capital próprio', q1)))
                mediana.append(get_('Financiamentos obtidos', med) /
                               (get_('Financiamentos obtidos', med) + get_('Capital próprio', med)))
                quartil_3.append(get_('Financiamentos obtidos', q3) /
                                 (get_('Financiamentos obtidos', q3) + get_('Capital próprio', q3)))

            elif st.session_state.option == 'Rentabilidade do Capital Investido':
                quartil_1.append(
                    (get_('EBIT', q1) - get_('Imposto', q1)) / get_('Ativo', q1))
                mediana.append(
                    (get_('EBIT', med) - get_('Imposto', med)) / get_('Ativo', med))
                quartil_3.append(
                    (get_('EBIT', q3) - get_('Imposto', q3)) / get_('Ativo', q3))

            elif st.session_state.option == 'Nível de Valor Acrescentado':
                quartil_1.append(get_('Valor acrescentado bruto (VAB)', q1) /
                                 (get_('Valor acrescentado bruto (VAB)', q1) +
                                 get_('CMVMC', q1) + get_('FSE', q1)))
                mediana.append(get_('Valor acrescentado bruto (VAB)', med) /
                               (get_('Valor acrescentado bruto (VAB)', med) +
                                get_('CMVMC', med) + get_('FSE', med)))
                quartil_3.append(get_('Valor acrescentado bruto (VAB)', q3) /
                                 (get_('Valor acrescentado bruto (VAB)', q3) +
                                 get_('CMVMC', q3) + get_('FSE', q3)))

            elif st.session_state.option == 'Turnover do Ativo':
                quartil_1.append(
                    get_('Volume de Negócios', q1) / get_('Ativo', q1))
                mediana.append(get_('Volume de Negócios', med) /
                               get_('Ativo', med))
                quartil_3.append(
                    get_('Volume de Negócios', q3) / get_('Ativo', q3))

            elif st.session_state.option == '% Rh no Volume de Negócios':
                quartil_1.append(get_('Gastos com Pessoal', q1) /
                                 get_('Volume de Negócios', q1))
                mediana.append(get_('Gastos com Pessoal', med) /
                               get_('Volume de Negócios', med))
                quartil_3.append(get_('Gastos com Pessoal', q3) /
                                 get_('Volume de Negócios', q3))

            elif st.session_state.option == '% FSE no Volume de Negócios':
                quartil_1.append(get_('FSE', q1) /
                                 get_('Volume de Negócios', q1))
                mediana.append(get_('FSE', med) /
                               get_('Volume de Negócios', med))
                quartil_3.append(get_('FSE', q3) /
                                 get_('Volume de Negócios', q3))

            elif st.session_state.option == '% CMVMC / Volume de negócios':
                quartil_1.append(get_('CMVMC', q1) /
                                 get_('Volume de Negócios', q1))
                mediana.append(get_('CMVMC', med) /
                               get_('Volume de Negócios', med))
                quartil_3.append(get_('CMVMC', q3) /
                                 get_('Volume de Negócios', q3))

            elif st.session_state.option == '% Custos no Volume de Negócios':
                quartil_1.append((get_('CMVMC', q1) + get_('FSE', q1) + get_('Gastos com Pessoal', q1)) /
                                 get_('Volume de Negócios', q1))
                mediana.append((get_('CMVMC', med) + get_('FSE', med) + get_('Gastos com Pessoal', med)) /
                               get_('Volume de Negócios', med))
                quartil_3.append((get_('CMVMC', q3) + get_('FSE', q3) + get_('Gastos com Pessoal', q3)) /
                                 get_('Volume de Negócios', q3))

            elif st.session_state.option == 'Taxa de Exportação':
                quartil_1.append(get_('Vendas e serviços prestados no mercado externo', q1) /
                                 get_('Volume de Negócios', q1))
                mediana.append(get_('Vendas e serviços prestados no mercado externo', med) /
                               get_('Volume de Negócios', med))
                quartil_3.append(get_('Vendas e serviços prestados no mercado externo', q3) /
                                 get_('Volume de Negócios', q3))

            else:
                quartil_1.append(get_(option_comparacao_varias.get(
                    st.session_state.option), q1))
                mediana.append(get_(option_comparacao_varias.get(
                    st.session_state.option), med))
                quartil_3.append(get_(option_comparacao_varias.get(
                    st.session_state.option), q3))

        return np.nanmean(quartil_1), np.nanmean(mediana), np.nanmean(quartil_3)
    ...
    with col2_text:
        ...
        st.markdown(
            f'<p class="title-font">{titulo_principal}</p>', unsafe_allow_html=True)
        st.markdown('<p class="sub_header-font">' +
                    st.session_state.option+'</p>', unsafe_allow_html=True)
        st.divider()

    with col1_mid:

        # anos selecionados no range slider do medidor
        range_years_gauge = list(
            range(year_slider_gauge[0], year_slider_gauge[1]+1, 1))

        # apresentação do titulo do medidor
        st.markdown(
            f'<p class="sub_header-font">{titulo_gauge}</p>', unsafe_allow_html=True)

        # média da empresa

        media_gauge_empresa = statistics.mean(st.session_state.df_comparacao.loc[st.session_state.df_comparacao.loc[
                                              st.session_state.df_comparacao['Indicador'] == st.session_state.option].index[0], range_years_gauge])*100

        # média do setor de todas as dimensões
        media_quartil_1, media_mediana, media_quartil_3 = medias_per_year(
            range_years_gauge)
        ...
        if st.session_state.option in ['% CMVMC / Volume de negócios']:
            ...
            if media_gauge_empresa == 0:
                ...
                st.markdown(
                    '<p class="sub_header-font"><b>Dados Indisponíveis</b></p>', unsafe_allow_html=True)
            elif media_gauge_empresa < media_quartil_1:
                st.image('images/high.png', use_column_width=True)
                st.markdown(
                    '<p class="sub_header-font"><b>Excelente</b></p>', unsafe_allow_html=True)
                ...
                st.markdown('<p class="sub_header-font"><b>Forte</b></p>',
                            unsafe_allow_html=True)
                ...
                st.markdown(
                    '<p class="sub_header-font"><b>Insuficiente</b></p>', unsafe_allow_html=True)

            elif media_gauge_empresa >= media_quartil_3:
                st.image('images/low.png', use_column_width=True)
                st.markdown('<p class="sub_header-font"><b>Fraco</b></p>',
                            unsafe_allow_html=True)
            else:
                st.image('images/nan.png', use_column_width=True)
                st.markdown(
    ...
    with col2_mid:
        ...
        range_years_linhas = list(
            range(year_slider_linhas[0], year_slider_linhas[1]+1, 1))

        # titulo do gráfico de linhas
        st.markdown(
            f'<p class="sub_header-font">{titulo_linhas}</p>', unsafe_allow_html=True)

        # valores da empresa do respetivo indicador selecionado, por ano
        value_empresa = st.session_state.df_comparacao.loc[st.session_state.df_comparacao['Indicador']
                                                           == st.session_state.option, st.session_state.df_comparacao.columns[3:]].iloc[0]
        ...
        if (st.session_state.option == 'Alavancagem Financeira'):
            ...
            value_media_setor = st.session_state.df_dados_setor_todas.loc[15, st.session_state.df_comparacao.columns[3:]] / (
                st.session_state.df_dados_setor_todas.loc[15, st.session_state.df_comparacao.columns[3:]] + st.session_state.df_dados_setor_todas.loc[8, st.session_state.df_comparacao.columns[3:]]) * 100

        else:
            # valores da média do setor, de todas as dimensões, do respetivo indicador selecionado, por ano
            value_media_setor = st.session_state.df_comparacao.loc[st.session_state.df_comparacao['Indicador']
                                                                   == st.session_state.option, st.session_state.df_comparacao.columns[3:]].iloc[1]
        ...
        # vai criar uma coluna para cada ano selecionado no range slider do gráfico de linhas
        row1_col2_mid = st.columns(len(range_years_linhas))
        for (col, year, value) in zip(row1_col2_mid, range_years_linhas, value_diferenca.loc[year_slider_linhas[0]:year_slider_linhas[1]]):
            ...
            col.markdown('<p class="text-font"><b>'+str(year) +
                         '</b></p>', unsafe_allow_html=True)

            if (~np.isnan(value)):
                # se tiver valor da diferença
                col.markdown('<p style="font-weight: 1000;font-size: 20px;">' +
                             str(round(value*100, 2))+' p.p</p>', unsafe_allow_html=True)
            else:
                # se não tiver valor da diferença
                col.markdown(
                    '<p style="font-weight: 1000;font-size: 20px;text-align:center;">Sem Dados</p>', unsafe_allow_html=True)
        ...
        line_fig = px.scatter(
            x=range_years_linhas, y=value_diferenca.loc[year_slider_linhas[0]:year_slider_linhas[1]]*100, trendline='ols')

        # o mode="lines" é para criar linhas entre os pontos
        line_fig.update_traces(line_color=color_1, line_width=5, mode="lines")
        ...
        line_fig.update_layout(
            xaxis=dict(
                ...
                ticktext=[
                    str(year)+"         " for year in st.session_state.df_comparacao.columns[3:]],
            ),
            legend=dict(
                orientation="h",
                yanchor="bottom",
    ...
    with col1_bot:
        ...
        range_years_barras = list(
            range(year_slider_barras[0], year_slider_barras[1]+1, 1))
        ...
        bar_fig.add_trace(go.Bar(
            x=range_years_barras, y=value_empresa.loc[year_slider_barras[0]:year_slider_barras[1]]*100, name="Empresa", marker=dict(color=color_2)))

        # dependendo da toggle box é apresentado um gráfico de barras ou um de linhas
        if toggle_barras_linhas:
            bar_fig.add_trace(go.Scatter(x=range_years_barras, y=value_media_setor.loc[year_slider_barras[
                              0]:year_slider_barras[1]]*100, name="Média do Setor", line={'width': 3}, marker=dict(color=color_3)))
        else:
            bar_fig.add_trace(go.Bar(x=range_years_barras, y=value_media_setor.loc[year_slider_barras[
                              0]:year_slider_barras[1]]*100, name="Média do Setor", marker=dict(color=color_3)))
        ...
        add_notation_to_fig(bar_fig, [year - 0.18 for year in range_years_barras],
                            value_empresa.loc[year_slider_barras[0]:year_slider_barras[1]], color_2, annotation)
        add_notation_to_fig(bar_fig, [year + 0.18 for year in range_years_barras],
                            value_media_setor.loc[year_slider_barras[0]:year_slider_barras[1]], color_3, annotation)
    ...
    with col2_bot:
        ...
        if media_gauge_empresa == 0:
            ...
            txt = "Em comparação com a média do setor nos anos avaliados do indicador <span style='font-weight: 1000;'><i>" + \
                st.session_state.option + \
                "</i></span>, o desempenho da empresa foi <b><i>Fraco</i></span> (entre os 25% inferiores da mesma dimensão)."

        elif media_gauge_empresa >= media_quartil_1 and media_gauge_empresa < media_mediana:
            txt = "Em comparação com a média do setor nos anos avaliados do indicador <span style='font-weight: 1000;'><i>"+st.session_state.option + \
                "</i></span>, o desempenho da empresa foi <span style='font-weight: 1000;'><i>Insuficiente</i></span> (entre os 50% inferiores da mesma dimensão)."

        elif media_gauge_empresa >= media_mediana and media_gauge_empresa < media_quartil_3:
            txt = "Em comparação com a média do setor nos anos avaliados do indicador <span style='font-weight: 1000;'><i>"+st.session_state.option + \
                "</i></span>, o desempenho da empresa foi <span style='font-weight: 1000;'><i>Forte</i></span> (entre os 50% superiores da mesma dimensão)."

        elif media_gauge_empresa >= media_quartil_3:
            txt = "Em comparação com a média do setor nos anos avaliados do indicador <span style='font-weight: 1000;'><i>"+st.session_state.option + \
                "</i></span>, o desempenho da empresa foi <span style='font-weight: 1000;'><i>Excelente</i></span> (entre os 25% superiores da mesma dimensão)."
        ...
        st.markdown(
            f'<p style="font-size:22px;">{txt}</p>', unsafe_allow_html=True)

    # botões para fazer o download da imagem, PDF e copiar para a clipboard a dashboard
    Print.buttons()

    st.markdown('<p style="text-align: right;" class="text-font">Fonte: Banco de Portugal</p>',
                unsafe_allow_html=True)
